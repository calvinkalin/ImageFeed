...
final class ImagesListService {
    ...
    private var apiURL = "https://api.unsplash.com/"
    ...
    private func makePhotoRequest(page: Int) -> URLRequest? {
        guard let url = URL(string: apiURL + "/photos?page=\(page)"),
               ...
               case .success(let decodedData):
                   ...
                   decodedData.forEach { dataOfPhoto in
                       let photo = Photo(
                           ...
                           createdAt: self.dateFormatterISO8601.date(from: dataOfPhoto.created_at ?? ""),
    ...
    private func likePhotoRequest(photoId: String) -> URLRequest? {
        guard let url = URL(string: apiURL + "photos/\(photoId)/like"),
              let token = OAuth2TokenStorage().token else {
